...
export default function InvestmentsClient({
  ...
  return (
    <div className="space-y-5 pb-24 lg:pb-8 animate-fade-in">
      ...
      {/* Investment Returns — moved here from the dashboard */}
      <InvestmentReturnsCard
        investments={filtered}
        totalInvested={totalInvested}
        totalCurrentVal={totalCurrent}
      />
      ...
      {filtered.length > 0 && (
        <div className="grid lg:grid-cols-3 gap-4">
          {/* Pie — portfolio breakdown (renamed from "Portfolio Composition";
              legend switched to $ values to match the card that used to live on the dashboard) */}
          <div className="card p-5">
            <h2 className="text-sm font-medium text-[var(--text-primary)] mb-1">Portfolio Breakdown<span className="ml-2 text-[10px] font-bold px-1.5 py-0.5 rounded" style={{color:'#0a0a12',background:'#60d4b4'}}>NEW HERE</span></h2>
              ...
              {byCategory.slice(0, 4).map((c) => (
                <div key={c.name} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="font-mono font-medium text-amber-400">
                    {formatCurrency(c.current)} · {totalCurrent > 0 ? ((c.current / totalCurrent) * 100).toFixed(0) : '0'}%
                  </span>
                </div>
              ))}
            </div>
          ...
          {/* Savings Rate — moved here from the dashboard, per request */}
          <SavingsRateCard totalIncome={monthIncomeTotal} totalExpenses={monthExpensesTotal} />
