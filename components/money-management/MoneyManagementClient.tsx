...
import {
  ...
  Pencil, Check, X, ChevronDown, PartyPopper,
} from 'lucide-react'
...
interface Props {
  userId: string
  currentMonth: string
  investments: { category: string; current_value: number }[]
...
type EditKey = 'income' | 'emergency' | 'rewards' | 'goal' | null
...
export default function MoneyManagementClient({
  userId, currentMonth, investments, expensesThisMonth, incomeThisMonth,
  ...
  const [openBreakdown, setOpenBreakdown] = useState<{ growth: boolean; essentials: boolean }>({ growth: false, essentials: false })
  ...
  const growthBreakdown = useMemo(() => {
    ...
    return Object.entries(map)
      .sort((a, b) => b[1] - a[1])
      .map(([name, value]) => ({ name, value, pct: growthActual > 0 ? Math.round((value / growthActual) * 100) : 0 }))
  }, [investments, growthActual])
  ...
  const essentialsBreakdown = useMemo(() => {
    ...
    return { rows, needsPct, wantsPct: 100 - needsPct }
  ...
  async function commitRewardsActual() {
    const value = parseNonNegativeAmount(editValue)
    if (value === null) { setEditError('Enter a valid amount (0 or more)'); return }
    setSaving(true)
  ...
  function toggleBreakdown(key: 'growth' | 'essentials') {
    setOpenBreakdown((prev) => ({ ...prev, [key]: !prev[key] }))
  }
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))' }}>
        ...
        <div className="card p-4" style={{ borderColor: 'rgba(251,191,36,0.2)' }}>
          ...
          {growthBreakdown.length > 0 && (
            <>
              <button onClick={() => toggleBreakdown('growth')} className="flex items-center gap-1 text-xs font-medium mt-2" style={{ color: BUCKET_META.growth.color }} aria-expanded={openBreakdown.growth}>
                Asset diversification <ChevronDown size={13} className={cn('transition-transform', openBreakdown.growth && 'rotate-180')} />
              </button>
              {openBreakdown.growth && (
        ...
        <div className="card p-4" style={{ borderColor: 'rgba(0,212,170,0.2)' }}>
          ...
          <div className="border-t pt-2.5 mt-1" style={{ borderColor: 'var(--border)' }}>
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <CycleDots month={cycleMonthNumber} total={EMERGENCY_CYCLE_LENGTH_MONTHS} />
              <span className="text-[10px] text-[var(--text-muted)]">
                {cycleComplete ? 'Cycle complete' : `Month ${cycleMonthNumber} of ${EMERGENCY_CYCLE_LENGTH_MONTHS} · resets in ${EMERGENCY_CYCLE_LENGTH_MONTHS - cycleMonthNumber} ${EMERGENCY_CYCLE_LENGTH_MONTHS - cycleMonthNumber === 1 ? 'month' : 'months'}`}
              </span>
            </div>
            ...
            {cycleComplete && (
              <div className="flex items-center justify-between gap-2 rounded-lg p-2" style={{ background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.25)' }}>
                <span className="flex items-center gap-2 text-xs text-[var(--text-secondary)]"><RefreshCw size={14} className="text-amber-400" /> 6-month cycle complete</span>
        ...
        {/* Essentials */}
        <div className="card p-4" style={{ borderColor: 'rgba(60,110,232,0.25)' }}>
          ...
          {essentialsBreakdown.rows.length > 0 && (
            <>
              <button onClick={() => toggleBreakdown('essentials')} className="flex items-center gap-1 text-xs font-medium mt-2" style={{ color: BUCKET_META.essentials.color }} aria-expanded={openBreakdown.essentials}>
                Expense breakdown <ChevronDown size={13} className={cn('transition-transform', openBreakdown.essentials && 'rotate-180')} />
              </button>
              {openBreakdown.essentials && (
                <div className="mt-2 pt-2 border-t" style={{ borderColor: 'var(--border)' }}>
                  <p className="text-[10px] text-[var(--text-muted)] mb-1.5">Needs {essentialsBreakdown.needsPct}% · Wants {essentialsBreakdown.wantsPct}%</p>
                  <div className="space-y-1.5">
                    {essentialsBreakdown.rows.map((row) => (
                      <div key={row.name} className="flex justify-between items-center text-xs">
                        <span className="flex items-center gap-1.5">
                          <span className="text-[var(--text-secondary)]">{row.name}</span>
                          <span className={cn('text-[9px] font-medium px-1.5 rounded-full', row.need ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-400/10 text-rose-300')}>
                            {row.need ? 'Need' : 'Want'}
                          </span>
                        </span>
                        <span className="font-mono text-[var(--text-secondary)]">{formatCurrency(row.value)}</span>
        ...
        <div className="card p-4" style={{ borderColor: 'rgba(236,72,153,0.2)' }}>
          ...
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs text-[var(--text-muted)]">This month <span className="text-[var(--text-secondary)]">· manual</span></span>
            {editing === 'rewards' ? (
              <EditRow value={editValue} onChange={(v) => { setEditValue(v); setEditError('') }} onCommit={commitRewardsActual} onCancel={cancelEdit} saving={saving} />
            ) : (
              <span className="flex items-center gap-1">
                <span className="font-mono text-sm font-medium text-[var(--text-primary)]">{formatCurrency(rewardsActual)}</span>
                <button onClick={() => startEdit('rewards', rewardsActual)} className="btn-ghost p-1"><Pencil size={12} /></button>
              </span>
            )}
          </div>
          {editError && editing === 'rewards' && <p className="text-rose-400 text-xs mb-1">{editError}</p>}
...
function CycleDots({ month, total }: { month: number; total: number }) {
  return (
    <span className="flex gap-1">
      {Array.from({ length: total }, (_, i) => (
        <span
          ...
          style={{ background: i < month ? '#00D4AA' : 'rgba(255,255,255,0.12)' }}
        />
      ))}
    </span>
  )
