...
import SpendingByMethodCard from './SpendingByMethodCard'
...
export default function ExpensesClient({
  ...
  return (
    <div className="space-y-5 pb-24 lg:pb-8 animate-fade-in">
      ...
      {/* Expenses by Category + Spending by Method — moved here from the dashboard.
          Both reflect the search/filter controls below, so they stay in sync with the table. */}
      <div className="grid md:grid-cols-2 gap-4">
        <ExpenseCategoryCard
          expenses={filtered}
          subtitle={hasActiveFilters ? 'Filtered view' : 'All time'}
          onAdd={() => setShowAddModal(true)}
        />
        <SpendingByMethodCard
          expenses={filtered}
          subtitle={hasActiveFilters ? 'Filtered view' : 'All time'}
          onAdd={() => setShowAddModal(true)}
        />
      ...
      {/* Recent Expenses — moved here from the dashboard */}
      <RecentExpensesCard expenses={filtered} onAdd={() => setShowAddModal(true)} />
