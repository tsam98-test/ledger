...
import IncomeBySourceCard from './IncomeBySourceCard'
import RecentIncomeCard from './RecentIncomeCard'
...
export default function IncomeClient({ initialIncome, userId }: { initialIncome: Income[]; userId: string }) {
  ...
  return (
    <div className="space-y-5 pb-24 lg:pb-8 animate-fade-in">
      ...
      {/* Income by Source — moved here from the dashboard */}
      <IncomeBySourceCard
        income={filtered}
        subtitle={hasFilters ? 'Filtered view' : 'All time'}
        onAdd={() => setShowAdd(true)}
      />

      {/* Recent Income — moved here from the dashboard */}
      <RecentIncomeCard income={filtered} onAdd={() => setShowAdd(true)} />
